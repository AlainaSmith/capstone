...
const getJarrodSoloEntry = () => axios.get(baseURL).then(jarrodSoloEntryCallback).catch(errCallback)
const createJarrodSoloEntry = body => axios.post(baseURL, body).then(jarrodSoloEntryCallback).catch(errCallback)
const deleteJarrodSoloEntry = (id) =>axios.delete(`${baseURL}/${id}`).then(jarrodSoloEntryCallback).catch(errCallback)
